    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="admin-page">
            <div className="admin-header">
                <div>
                    <p className="admin-kicker">Approvals</p>
                    <h1 className="admin-title">Change Requests</h1>
                    <p className="admin-subtitle">{requests.length} {filter === 'pending' ? 'pending' : 'total'} request{requests.length !== 1 ? 's' : ''}</p>
                </div>
                    ...
                    {requests.map(r => {
                        ...
                        return (
                            <motion.div
                                ...
                                className="card-savron p-0 overflow-hidden"
                            >
                                <button
                                    onClick={() => setExpanded(isExpanded ? null : r.id)}
                                    className="w-full px-6 py-5 flex items-center justify-between gap-4 text-left hover:bg-white/[0.025] transition-colors"
                                >
