...
import { Trash2, ExternalLink, ChevronDown, UserCheck, Clock, X, FileVideo, Phone, Instagram, RefreshCw } from 'lucide-react';
import { cn } from '@/lib/utils';
import { format } from 'date-fns';
import type { Applicant } from '@/lib/types';
import {
    deleteApplicantAction,
    deleteApplicantByEmailAction,
...
export default function AdminApplicantsClient({
    ...
    }, [initialApplicants, initialLoadError]);
    ...
    const updateStatus = async (id: string, status: Applicant['status']) => {
        try {
            const updated = await updateApplicantStatusAction(id, status);
            setApplicants(prev => prev.map(a => a.id === id ? updated : a));
            setSelectedApplicant(prev => prev && prev.id === id ? updated : prev);
            setLoadError('');
        } catch (err) {
    ...
    const purgeByEmail = async () => {
        ...
        setLoadError('');
        try {
            await deleteApplicantByEmailAction(email);
            setApplicants(prev => prev.filter(a => a.email.toLowerCase() !== email));
            setPurgeEmail('');
        } catch (err) {
            setLoadError(err instanceof Error ? err.message : 'Failed to remove application for that email.');
        } finally {
            setPurging(false);
        }
    };
            ...
            <div className="admin-header">
                <div>
                    ...
                    <p className="admin-subtitle">
                        {visibleApplicants.length} application{visibleApplicants.length !== 1 ? 's' : ''} shown · {applicants.length} total in database
                    </p>
                </div>
                <div className="flex items-center gap-2">
                    <button
                        ...
                        disabled={refreshing}
                        className="px-4 py-2 text-xs uppercase tracking-widest border border-white/20 text-white bg-white/5 hover:bg-white/10 rounded-savron transition-all font-medium disabled:opacity-50"
                    >
                                            ...
                                            {openDropdown === a.id && (
                                                ...
                                                >
                                                    {STATUS_OPTIONS.filter(s => s !== a.status).map(s => (
                                                        <button
                                                            key={s}
                                                            onClick={e => { e.stopPropagation(); void updateStatus(a.id, s); setOpenDropdown(null); }}
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="flex gap-3">
                                ...
                                <button
                                    onClick={() => void deleteApplicant(confirmDelete)}
                                    ...
                                    <div className="space-y-2 pt-2">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {STATUS_OPTIONS.map(s => (
                                                <button
                                                    key={s}
                                                    onClick={() => void updateStatus(selectedApplicant.id, s)}
                                                    className={cn(
